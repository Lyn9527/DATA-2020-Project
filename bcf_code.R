...
# setwd("/Users/zhouwenjun/Desktop/DATA 2020 HW/Final Project") # Uncomment and set your path

## ── 1. Packages ───────────────────────────────────────────────────────────
# Ensure packages are installed and loaded
pkgs <- c("bcf", "SuperLearner", "tidyverse", "doParallel", "data.table", "stats")
...
for (rep in 1:N_REPETITIONS) { # Loop through the 50 repetitions
  ...
  run_results <- foreach(
    ...
    # --- SuperLearner ---
    sl_result <- tryCatch({
      sl_fit <- SuperLearner(Y = z, X = as.data.frame(X), family = binomial(), SL.library = c("SL.glm", "SL.gbm"))
      ps <- sl_fit$SL.predict; ps <- pmin(pmax(ps, 1e-6), 1 - 1e-6); if(any(is.na(ps))) ps[is.na(ps)] <- 0.5
      list(ps = ps)
    }, error = function(e) { list(error = paste("SuperLearner failed:", e$message)) })
    if ("error" %in% names(sl_result)) return(c(list(tag = nm, rep = rep), sl_result))
    ps <- sl_result$ps
